...
pwd=`pwd`
ci=`sudo docker run -d -v $pwd:/var/lib/egov-services:rw -w /var/lib/egov-services --net=container:${db} ${ci_image} bash -c "cd $1; mvn clean verify package -s /var/lib/egov-services/settings.xml"`
